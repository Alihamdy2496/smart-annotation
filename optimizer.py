...
def create_non_overlap_constraints(movables, fixed_obstacles, min_separation=0.0, 
    ...
    for i in range(n_movables):
        for j in range(i+1, n_movables):
            ...
            adaptive_margin = min_separation + (margin_ratio * avg_size if margin_ratio is not None else 0.0)
            ...
            constraints.append({
                'type': 'ineq',
                'fun': make_constraint_movable(i, j, adaptive_margin)
            })
...
def create_positive_side_constraints(movables, fixed_obstacles):
    ...
    for i in range(n_movables):
        ...
        for obs_idx, obs in enumerate(fixed_obstacles):
            ...
            # normal = vec_to_target_norm # np.array([-vec_to_target_norm[1], vec_to_target_norm[0]])
            
            # Capture loop variables using default arguments (copy normal to avoid reference issues)
            normal_copy = vec_to_target.copy()
            center_copy = center.copy()
            
            def make_positive_side_constraint(i_val=i, obs_center=center_copy, normal_vec=normal_copy):
                def constraint(xvec):
                    pts = unpack_xy(xvec)
                    p = pts[i_val]
                    ...
                    return np.dot(vec_to_movable, normal_vec)
                return constraint
...
def optimize(movables, fixed_obstacles, num_restarts=20, maxiter=400, 
             placement_bounds=None, min_separation=0.0, margin_ratio=0.1,
             target_weight=100.0, normal_weight=10.0):
    """
    ...
    overlap_constraints = create_non_overlap_constraints(movables, fixed_obstacles, 
                                                         min_separation, margin_ratio)
    
    # Create positive side constraints (movables must be in front of fixed obstacles)
    positive_side_constraints = create_positive_side_constraints(movables, fixed_obstacles)
    
    # Combine all constraints
    constraints = overlap_constraints + positive_side_constraints
    ...
    def check_feasibility(x, constraints_list, verbose=False):
        ...
        for idx, constraint in enumerate(constraints_list):
            ...
            if verbose and val < -1e-6:
                constraint_type = "overlap" if idx < len(overlap_constraints) else "positive-side"
    ...
    target_feasible, target_min_const, _ = check_feasibility(x_targets, constraints, verbose=False)
    print(f"Target positions feasibility: {'FEASIBLE' if target_feasible else 'INFEASIBLE'} (min constraint: {target_min_const:.6f})")
    ...
    for restart_idx in range(num_restarts):
        ...
        for init_attempt in range(max_init_attempts):
            ...
            for i, m in enumerate(movables):
                ...
                if init_attempt == 0:
                    # First attempt: start at target
                    jitter = np.zeros(2)
                elif init_attempt < 5:
                    # Next attempts: small random jitter
                    jitter = 0.3 * (np.random.rand(2) - 0.5)
                else:
                    # Later attempts: larger exploration
                    jitter = 1.5 * (np.random.rand(2) - 0.5)
                x0_candidate.append(np.array([tx, ty]) + jitter)
            x0_candidate = np.array(x0_candidate).reshape(-1)
            
            # Check if initial point is feasible
            is_feasible, min_const, violations = check_feasibility(x0_candidate, constraints, verbose=False)
            if is_feasible or init_attempt == max_init_attempts - 1:
                x0 = x0_candidate
                if not is_feasible:
                    print(f"Restart {restart_idx+1}: Starting from infeasible point (min constraint: {min_const:.6f})")
                break
        ...
        try:
            ...
            is_feasible, min_constraint, violations = check_feasibility(res.x, constraints, verbose=False)
            ...
            if is_feasible:
                ...
                if restart_idx == 0:  # Only show details for first restart
                    print(f"Restart {restart_idx+1}: Infeasible solution (min constraint: {min_constraint:.6f})")
                    print(f"  Most violated constraints:")
                    sorted_violations = sorted(enumerate(violations), key=lambda x: x[1])[:5]
